...
rm -rf $REPO_DIR
mkdir -p $REPO_DIR
ln -s /datasets/refcats/htm/ $REPO_DIR/ref_cats

echo "lsst.obs.ctio0m9.Ctio0m9Mapper" > $REPO_DIR/_mapper

## The data on the following paths are all duplicates, or have the wrong size CCD
## and are thus incompatible with the current version (up-to and including DM-10565) of this obs package
## we therefore do not ingest them.
# /lsst2/photocalData/data/ctio0m9_sanitised/f*.fits --badFile fitstest.fits
# /lsst2/photocalData/data/ctio0m9_sanitised/march2016/*/*.fits
